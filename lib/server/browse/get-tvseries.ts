...
export const getTvSeries = async (
  tvSeriesSearchParams: TVSeriesSearchParams
) => {
  ...
  //page
  const page = tvSeriesSearchParams.page ?? "1";
  processedParams.append("page", page);

  //with_genres
  const genres = tvSeriesSearchParams.genres;
  if (genres) {
    processedParams.append("with_genres", genres);
  }

  //sortby
  const sortBy = tvSeriesSearchParams.sortBy;
  ...
  //filter_by
  const filterby = tvSeriesSearchParams.filterBy;
