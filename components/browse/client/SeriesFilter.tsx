...
import {
  TVSeriesFilterByTypeValuesObj,
  TVSeriesFilterByType,
...
export const SeriesFilter: FC = () => {
  ...
  const [selectedFilter, setSelectedFilter] =
    useState<TVSeriesFilterByType | null>(
      readOnlySearchParams.get("filterBy") as TVSeriesFilterByType | null
    );

  const handleChange = useCallback(
    (value: TVSeriesFilterByType) => {
  ...
  return (
    <fieldset>
      ...
      {Object.values(TVSeriesFilterByTypeValuesObj).map((val, i) => (
        <div key={`${i}-${val}`}>
          <input
            type="radio"
            id={`${i}-${val}`}
